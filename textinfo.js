;(function($)
{
    ...
    BigText = {
        ...
        init: function($head)
        {
            if(!$('#'+BigText.GLOBAL_STYLE_ID).length) {
                $head.append(BigText.generateStyleTag(BigText.GLOBAL_STYLE_ID, ['.textinfo { font-size: ' + BigText.STARTING_PX_FONT_SIZE + 'px; }']));
            }
        },
    ...
    $.fn.textinfo = function(options)
    {
        ...
        return this.each(function()
        {
            ...
            if(!id) {
                id = 'textinfo-id' + (BigText.counter++);
                $t.attr('id', id);
            }
            ...
            if(options.resize) {
                ...
                {
                    var $t = $('#'+id);
                    $t.textinfo($t.data(BigText.DATA_KEY));
                }
                ...
                if($.isFunction(options.bindResize)) {
                    ...
                    $(window).unbind('resize.textinfo').bind('resize.textinfo', $.throttle(100, resizeFunction));
                } else if($.fn.smartresize) {
                    // https://github.com/lrbabe/jquery-smartresize/
                    $(window).unbind('smartresize.textinfo').bind('smartresize.textinfo', resizeFunction);
                }
            }
